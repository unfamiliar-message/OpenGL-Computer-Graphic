...
bool TriangleMesh::LoadFromFile(const std::string& filePath, const bool normalized)
{
    ...
    if (normalized) {
        ...
        while (std::getline(fin, line))
        {
            ...
            if (type == "vt") { //preserved feature
                sin >> x >> y;
                vertices_texture.push_back(glm::vec2(x, y));
            }
            if (type == "vn") {  //normal
            ...
            if (type == "f") {
                ...
                do {
                    ...
                    vertices_texture_index.push_back(abs(vt0) - 1);
                    vertices_texture_index.push_back(temp_vt - 1);
                    vertices_texture_index.push_back(abs(vt2) - 1);
...
bool TriangleMesh::LoadFrom_mtlfile(const std::string& filePath, const bool normalized) {
    std::string res, line;
    std::ifstream fin(filePath);
    PhongMaterial temp;//PhongMaterail need it
    if (!fin.is_open())
    {
        std::cout << "mtl file: " << filePath << " fails to open." << std::endl;
        exit(-1);
    }
    if (normalized) {
        std::string mtltype;
        std::string type;
        std::string kd_map_type;
        while (std::getline(fin, line))
        {
            ...
            if (type == "map_Kd") {
                ...
                ImageTexture* kd_map = new ImageTexture(kd_map_type);
                temp.SetMapKd(kd_map);
                std::cout << temp.GetMapKd() << std::endl;
                std::cout << "map_Kd:" << kd_map_type << std::endl;

                phongmaterialpair[mtltype] = temp;
            }
            type = "";
        }
    ...
    for (int i = 0; i < subMeshes.size(); i++) {
        for (int j = 0; j < subMeshes[i].vertexIndices.size(); j++) {
            ...
            if (vertices[subMeshes[i].vertexIndices[j]].normal == glm::vec3(0.0f, 1.0f, 0.0f)) {
                ...
                vertices[subMeshes[i].vertexIndices[j]].texcoord = vertices_texture[vertices_texture_index[count + j]];
            }
            else {
                VertexPTN ptn;
                ptn.position = vertices[subMeshes[i].vertexIndices[j]].position;
                ptn.texcoord = vertices_texture[vertices_texture_index[count + j]];
